...
const PLAN_SCHEMA = {
  ...
  required: ["name", "interpretation", "onceFields", "list", "itemFields", "columns", "cell", "itemLayout", "headerLayout", "palette", "checks"]
};
...
function normalise(dsl) {
  ...
  const contentBottom = Math.max(...dsl.elements.filter((e) => e.kind !== "box").map((e) => (Number(e.y) || 0) + (Number(e.h) || 6)), 12) + 2;
  ...
  dsl.elements = dsl.elements.map((el) => {
    const e = { ...el, x: Math.max(0, Number(el.x) || 0), y: Math.max(0, Number(el.y) || 0), w: Math.max(2, Number(el.w) || 10), h: Math.max(0.3, Number(el.h) || 6) };
