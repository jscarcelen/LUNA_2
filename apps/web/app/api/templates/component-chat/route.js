...
export async function POST(request) {
  try {
    ...
    const plan = await chat(apiKey, {
      ...
      messages: [...(previous ? [{ role: "assistant", content: `Current component: ${JSON.stringify(previous)}` }] : []), image ? { role: "user", content: [{ type: "text", text: `${prompt}\n\nUse the attached image as the visual reference: reproduce its structure, shapes, proportions and colour feel (not its exact words).` }, { type: "image_url", image_url: { url: image, detail: "high" } }] } : { role: "user", content: prompt }]
    });
