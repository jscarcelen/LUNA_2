...
const BRIEF_SCHEMA = {
    ...
    sections: {
      ...
      items: {
        ...
        properties: {
          ...
          request: { type: "string", description: "A complete component request for the designer: what it contains, which fields the AI fills, how it looks, sizes. Self-contained — the designer does not see the rest." },
  ...
  required: ["name", "improvedPrompt", "canvas", "accent", "reuseTemplateName", "sections", "views", "images", "notes"]
};
...
${DESIGN_RULES}`,
    ...
    const designed = await Promise.all(sections.map(async (section) => {
      const context = `This component is the "${section.title}" (${section.role}) of a document: ${brief.improvedPrompt}. Accent colour ${brief.accent || "#5b5bd6"}. ${section.repeats ? "It repeats once per generated item." : "It appears once."}`;
      try {
        const { dsl } = await designComponent({ apiKey, prompt: `${section.request}\n\nDocument: ${brief.improvedPrompt}`, image: section.role === "content" ? image : "", context });
    ...
    return NextResponse.json({
      brief: { name: brief.name, canvas: brief.canvas, accent: brief.accent, views: brief.views, improvedPrompt: brief.improvedPrompt, reuseTemplateName: brief.reuseTemplateName, images: brief.images, notes: brief.notes },
